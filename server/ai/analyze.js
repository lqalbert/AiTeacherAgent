...
import { normalizeEvaluationPoints } from './evaluationPoints.js'
...
4. 评分维度请输出：${dimLabels}
5. **证据引用（强制）**：每条 strengths / improvements 必须是对象，含 claim；并尽量给出 page（证据中的「第 N 页」页码，1 起）与 quote（摘自该页【教师讲解】或【课件内容】的原句/近原句，≤40字）。找不到可靠页码或摘录时可省略 page/quote，但不得编造未出现的原文。
...
严格返回 JSON：
{
  ...
  "score": 3,
  "strengths": [
    { "claim": "具体亮点", "page": 2, "quote": "支撑该亮点的转写或课件摘录" }
  ],
  "improvements": [
    { "claim": "具体改进建议", "page": 5, "quote": "支撑该建议的转写或课件摘录" }
  ],
  "dimensions": ${JSON.stringify(dimJson)}
}

score 与 dimensions 均为 1-5 整数；strengths 2-4 条，improvements 2-5 条。`

  const content = await chatCompletion(
    [
      {
        role: 'system',
        content: `${buildStrictSystem(config)}\n评价必须基于证据与用户配置标准，拒绝空洞表扬。每条建议尽量附页码与原文摘录。输出合法 JSON。`,
      },
      { role: 'user', content: prompt },
    ],
  ...
  return {
    ...
    strengths: normalizeEvaluationPoints(parsed.strengths),
    improvements: normalizeEvaluationPoints(parsed.improvements),
    dimensions,
  }
}
...
export async function runFullAnalysis(sessionStore, sessionId, roundNumber = null) {
  ...
  let questions = []
  if (sections.homework?.enabled !== false) {
